...
/**
 * Back-office — Mon compte (Employee / SuperAdmin).
 *
 ...
 *
 * Reprend la même logique que ClientMonCompteComponent (espace
 * client), avec deux différences volontaires :
 * - bloc matricule/rôle : renseigné uniquement pour un Employee
 *   (toujours vide/masqué pour un SuperAdmin — voir
 *   MonCompteController.toDto côté backend) ;
 * - pas de bloc adresse/allergie/préférences/fidélité : ce sont des
 *   champs ClientAuthentifie (voir MonCompteDTO côté backend), qui
 *   restent null pour un Employee/SuperAdmin et n'ont pas leur place
 *   ici.
 */
@Component({
...
export class MonCompteComponent implements OnInit {
  ...
  // Champs propres à l'Employee (voir MonCompteDTO côté backend) :
  // lecture seule, jamais envoyés dans le payload d'update. Restent
  // vides pour un SuperAdmin.
  matricule = '';
  nomRole = '';
  ...
  onPhotoSelectionnee(event: Event): void {
    ...
    this.monCompteService.uploadPhoto(fichier).subscribe({
      ...
      error: (error) => {
        ...
        this.uploadPhotoEnCours = false;
        this.errorMessage =
          error?.error?.message || "Impossible d'uploader la photo. Réessayez.";
        input.value = '';
        this.cdr.detectChanges();
      },
    });
